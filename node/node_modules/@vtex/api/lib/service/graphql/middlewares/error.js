...
const getSplunkQuery = (account, workspace) => `Try this query at Splunk to retrieve error log: 'index=colossus key=log_error sender="${sender}" account=${account} workspace=${workspace}'`;
const parseMessage = ramda_1.pluck('message');
...
async function graphqlError(ctx, next) {
    const { vtex: { account, workspace, route: { id, }, }, } = ctx;
    ...
    try {
        ...
        if (graphQLErrors) {
            ...
            ramda_1.forEach((err) => {
                ...
                let level = err.originalError && err.originalError.level;
